...
import { ArrowLeft, Info, CalendarDays, Globe, Check } from "@lucide/vue";
import { useSettingsWindow } from "../composables/useSettingsWindow";
import { searchUsageStore, loadSearchUsage } from "../stores/config";
import type { SearchUsageRecord } from "../stores/config";
...
onMounted(() => {
  ...
  loadSearchUsage(true);
});

// ── View state ──
type ViewKey = "date" | "provider";
...
const providerLabels = computed<Map<string, string>>(() => {
  ...
  for (const [k, n] of names) {
    const preset = k.includes("|") ? k.split("|")[0] : "";
    out.set(k, (counts.get(n) ?? 0) > 1 && preset ? `${n} (${preset})` : n);
  }
  return out;
});

/** Date view window: 7 or 30 days. */
const windowRecords = computed<SearchUsageRecord[]>(() => {
...
const dateRecords = computed<SearchUsageRecord[]>(() =>
...
// ── Provider view: donut groups (fixed 30-day window) ──
interface Group {
  key: string;
  label: string;
  requests: number;
...
const providerGroups = computed<Group[]>(() => {
  const map = new Map<string, Group>();
  for (const r of records30.value) {
    const key = providerKeyOf(r);
    let g = map.get(key);
    if (!g) {
      g = { key, label: providerLabels.value.get(key) ?? r.provider, requests: 0 };
      map.set(key, g);
    }
    g.requests += 1;
  }
  const groups = [...map.values()].sort((a, b) => b.requests - a.requests);
  // Keep the legend readable: top 8 slices, the tail folds into "Other".
  if (groups.length <= 8) return groups;
  const head = groups.slice(0, 8);
  head.push({
    key: "__other__",
    label: t("settings.usageStats.other"),
    requests: groups.slice(8).reduce((s, g) => s + g.requests, 0),
  });
  return head;
});
...
const donut = computed<{ total: number; segments: DonutSeg[] }>(() => {
  const groups = providerGroups.value;
  const total = groups.reduce((s, g) => s + g.requests, 0);
  let acc = 0;
  const segments = groups.map((g, i) => {
    const pct = total > 0 ? g.requests / total : 0;
    const seg: DonutSeg = {
      group: g,
      pct,
      dash: pct * DONUT_C,
...
<template>
  <div class="ud-root" :class="{ 'grow-above': growAbove }" @mousedown="handleDrag">
    ...
    <div class="ud-header">
      ...
      <span class="header-title">{{ t('settings.searchUsage.pageTitle') }}</span>
    </div>
    ...
    <div class="ud-body">
      ...
      <div class="usage-seg">
        ...
        <button :class="{ active: view === 'provider' }" @click="view = 'provider'">
          <Globe :size="12" :stroke-width="1.9" />{{ t('settings.searchUsage.viewProvider') }}
        </button>
      </div>
      ...
      <template v-else-if="view === 'date'">
        ...
        <div v-else class="chart-card">
          <div class="chart-head">
            <span class="chart-title">{{ t('settings.usageStats.dailyRequests') }}</span>
            <span class="chart-total">{{ t('settings.usageStats.total', { n: t('settings.usageStats.requests', dateRecords.length) }) }}</span>
          </div>
          <div class="bars-row" :key="days">
            <div v-for="(b, i) in dailyBuckets" :key="i" class="bar-cell">
              <div
                class="bar"
                :style="{ height: barHeight(b.requests, Math.max(...dailyBuckets.map(x => x.requests))) }"
                :title="`${b.label} · ${t('settings.usageStats.requests', b.requests)}`"
              />
            </div>
          </div>
      ...
      <!-- ─── By provider (request share donut) ─── -->
      <template v-else>
        <div class="chart-card">
          <div class="chart-head">
            <span class="chart-title">{{ t('settings.usageStats.requestsShare') }}</span>
            <span class="chart-total">{{ t('settings.usageStats.total', { n: t('settings.usageStats.requests', donut.total) }) }}</span>
          </div>
          <div class="donut-layout">
            <div class="donut-wrap">
              <svg viewBox="0 0 140 140" class="donut-svg">
                ...
                />
                <text x="70" y="67" text-anchor="middle" class="donut-total">{{ fmt(donut.total) }}</text>
                <text x="70" y="82" text-anchor="middle" class="donut-metric">{{ t('settings.usageStats.requestsMetric') }}</text>
              </svg>
            </div>
            <div class="legend">
              <div v-for="seg in donut.segments" :key="seg.group.key" class="legend-row">
                <span class="legend-dot" :style="{ opacity: seg.opacity }" />
                <span class="legend-name">{{ seg.group.label }}</span>
                <span class="legend-pct">{{ pctText(seg.pct) }}</span>
                <span class="legend-stats">{{ t('settings.usageStats.requests', seg.group.requests) }}</span>
              </div>
            </div>
          </div>
        </div>
...
/* Bar chart — bars and labels in separate rows so the x-axis is one
