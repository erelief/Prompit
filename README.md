...
Prompit 是一个常驻后台的悬浮小窗。**需要它的时候按一下快捷键**，它在光标旁边旁出现，借助 AI 模型视线一些简单的功能——翻译、润色、回答一个问题——然后把结果直接塞回光标所在的位置，自己消失。
shu'j
...
- **翻译人格**：设定“译者”的身份，更准确的符合你的翻译场景
- **用户词典**：可以自己维护的词典，把一些单词翻译成固定的内容

### ✨ 轻技能（Skills Lite）
除了翻译，还可以把一些常用系统提示词作为预置模板放入软件内，相当于一个只能接入 SKILL.md (真的支持直接导入) 的单文件技能系统。
- 预置简单问答技能（quick-qustion）：AI 用简要的答案回复你的问题
- 预置 XML 格式化提示词金技能（xml-tag-prompt）：用 XML 标签的形式优化你的提示词，更有条理，AI 回自动补充缺失信息
...
- **Prompit 目前已有功能完全免费**，没有付费功能、没有内购、没有订阅。所有费用都由你和第三方服务供应商产生。如果你使用自部署模型，则完全隐私，且不会产生任何费用。
...
Prompit **不是** AI 输入法，它更像 [Alfred](https://www.alfredapp.com/) 或 [Listray](https://www.listary.com/) 那样的快捷启动工具。

现代 AI 输入法一般强调了“语音输入”的效率，可是，即便是在独处的情况下，对着电脑自言自语对我来说仍旧非常尴尬；AI 输入法也缺少“边界感”，我只想要“我需要 AI 来的时候”，AI 才来。所以，Prompit 和输入法的生态位并不冲突。
...
## 安全说明
本项目全部由 Vibe Coding 而成，我本人可以说是不会编程，但我在能力范围内尽可能的理解和学习下，软件有一定基本的安全需求（所有个人数据都是加密存储，非明文暴露，并做了简单的数据导入导出和销毁），但仍然请注意以下内容：
- 本软件可能存在不完善的地方
- 使用官方渠道提供的服务，警惕使用第三方中转服务
- **不要将隐私、重要或高风险的信息**提供给 AI 服务
- 使用**独立的** API key 用于该软件，并定期跟换
- 定期清理历史记录
